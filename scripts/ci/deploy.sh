...
METADATA_VERSION=v$(grep -Po "(?<=^version=).*" projectgenerator/metadata.txt)
...
./scripts/ci/plugin_upload.py -u "${OSGEO_USERNAME}" -w "${OSGEO_PASSWORD}" -r"${TRAVIS_TAG}" package.zip

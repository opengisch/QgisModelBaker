...
def test_ilimodels_xml_parser_23():
   ilicache = IliCache([])
   ilicache._process_ilimodels(os.path.join(test_path, 'testdata', 'ilirepo', '23', 'ilimodels.xml'), 'test_repo')
...
def test_ilimodels_xml_parser_24():
   ilicache = IliCache([])
   ilicache._process_ilimodels(os.path.join(test_path, 'testdata', 'ilirepo', '24', 'ilimodels.xml'), 'test_repo')
   assert 'test_repo' in ilicache.repositories.keys()
   models = set([e['name'] for e in next(elem for elem in ilicache.repositories.values())])

...
class GenerateProjectDialog(QDialog, DIALOG_UI):
    ...
    def accepted(self, edited_command=None):
        ...
        with OverrideCursor(Qt.WaitCursor):
            ...
            if "qgis.modelbaker.qml" in self.metaconfig.sections():
                self.print_info(
                    self.tr("Metaconfig contains QML toppings."), COLOR_TOPPING
                )
    ...
    def load_metaconfig(self):
        ...
        if "ch.ehi.ili2db" in self.metaconfig.sections():
            ...
            if "iliMetaAttrs" in ili2db_metaconfig:
                self.print_info(
                    self.tr("- Seek for iliMetaAttrs (toml) files:"), COLOR_TOPPING
                )
                ...
                self.print_info(
                    self.tr("- Loaded iliMetaAttrs (toml) files"), COLOR_TOPPING
                )

            # get prescript (sql)
            if "prescript" in ili2db_metaconfig:
                self.print_info(
                    self.tr("- Seek for prescript (sql) files:"), COLOR_TOPPING
                )
                ...
                self.print_info(
                    self.tr("- Loaded prescript (sql) files"), COLOR_TOPPING
                )

            # get postscript (sql)
            if "postscript" in ili2db_metaconfig:
                self.print_info(
                    self.tr("- Seek for postscript (sql) files:"), COLOR_TOPPING
                )
                ...
                self.print_info(
                    self.tr("- Loaded postscript (sql) files"), COLOR_TOPPING
                )
    ...
    def get_topping_file_model(self, id_list):
        ...
        if len(topping_file_cache.downloaded_files) == len(id_list):
            self.print_info(
                self.tr("- - All topping files successfully downloaded"), COLOR_TOPPING
            )
        else:
